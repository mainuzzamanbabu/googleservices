...
def scrape_multiple_sites_parallel(query, max_sites=2, max_total_time=60, max_search_results=15):
    """
    Scrape multiple sites in parallel within the same time budget
    """
    ...
    # Phase 1: Parallel scraping of first batch
    print(f"\nPhase 1: Parallel scraping of first {min(6, len(filtered_urls))} sites...")
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        # Submit first 6 URLs for parallel scraping
        batch_urls = filtered_urls[:6]
        future_to_url = {
            executor.submit(try_scrape_smart, url, 10): url 
            for url in batch_urls
        }
        
        # Calculate remaining time for this phase
        time_for_phase1 = max_total_time * 0.7  # Use 70% of total time for phase 1
        
        print(f"  Submitting {len(batch_urls)} URLs for parallel processing (timeout: {time_for_phase1:.1f}s)")
        
        # Collect results as they complete
        try:
            for future in concurrent.futures.as_completed(future_to_url, timeout=time_for_phase1):
                if len(successful_scrapes) >= max_sites:
                    print(f"  ✓ Target of {max_sites} sites reached, stopping early")
                    break
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"  ✓ Parallel success from {get_domain(url)} ({len(successful_scrapes)}/{max_sites})")
                    else:
                        print(f"  ✗ Parallel failure from {get_domain(url)}")
                except Exception as e:
                    print(f"  ✗ Parallel scraping error for {get_domain(url)}: {e}")
        
        except concurrent.futures.TimeoutError:
            print(f"  ⏰ Phase 1 timeout after {time_for_phase1:.1f}s")
    
    # Phase 2: Playwright fallback for remaining sites if needed
    remaining_time = max_total_time - (time.time() - total_start)
    if len(successful_scrapes) < max_sites and remaining_time > 15 and PLAYWRIGHT_AVAILABLE:
        print(f"\nPhase 2: Playwright fallback for remaining sites (time remaining: {remaining_time:.1f}s)")
        
        # Try remaining URLs with Playwright
        remaining_urls = filtered_urls[6:9]  # Next 3 URLs
        ...
        for i, url in enumerate(remaining_urls):
            ...
            remaining_time = max_total_time - (time.time() - total_start)
            if remaining_time < 10:  # Need at least 10s for Playwright
                print(f"  ⏰ Insufficient time remaining ({remaining_time:.1f}s)")
                break
            
            print(f"  Playwright attempt {i+1}: {get_domain(url)}")
            scraped = try_playwright_scrape_smart(url, min(15, remaining_time - 2))
            if scraped:
                successful_scrapes.append(scraped)
                print(f"  ✓ Playwright success ({len(successful_scrapes)}/{max_sites})")
...
if __name__ == "__main__":
    ...
    print(f"Average time per site: {execution_time/len(results):.2f} seconds" if results else "N/A")
    print(f"="*60)

# def scrape_bulk_products(product_queries, output_csv="bulk_scraping_results.csv"):
#     """
#     Scrape multiple products in bulk and save to CSV
#     """
#     print("Bulk Smart Web Scraper with CSV Output")
#     print("=" * 60)
    
#     # Prepare CSV file
#     csv_headers = ['query', 'url', 'scraped_content', 'total_time', 'status', 'timestamp']
...
#             execution_start = time.time()
#             result = scrape_single_site(query)
#             execution_time = time.time() - execution_start
...
#             if result:
#                 row_data['url'] = result['url']
#                 row_data['scraped_content'] = json.dumps(result['content'], ensure_ascii=False)
...
#                 print(f"✗ FAILED: Could not scrape any data")
#                 print(f"  Time: {execution_time:.2f}s")
...
#     # Option 1: Run all products
#     scrape_bulk_products(product_queries)