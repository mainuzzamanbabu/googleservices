...
def scrape_single_site(query, max_search_results=10):
    """
    ...
    with DDGS() as ddgs:
        results = [r for r in ddgs.text(query, max_results=max_search_results)]
    ...
    print(f"Filtered to {len(filtered_results)} unique domains")
    ...
    for i, url in enumerate(filtered_results[:5]):
        print(f"\nAttempt {i+1}: {url}")
    ...
    if PLAYWRIGHT_AVAILABLE:
        ...
        for i, url in enumerate(filtered_results[:3]):
...
if __name__ == "__main__":
    print("Smart Web Scraper - Single Site with Optimized Data")
    print("=" * 60)
